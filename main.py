...
gender_model = load_model(dir_path + '/gender_classifier.h5')
...
with col2:
    radio = st.radio('Mode', ['Upload', 'Live(Available when you run it on your own PC*)'])
    confidence = st.slider("How confident should the model be that it's seeing a face?", 0, 10, 5)
...
def look_for_faces_update_text(passedframe):
    faces = face_detect.detectMultiScale(passedframe, minNeighbors=confidence)
    for (x, y, w, h) in faces:
        face_img = passedframe[y:y + h, x:x + w]
        ...
        gender_prediction = (gender_model.predict(reshaped2) > 0.5).astype("int32")[0][0]
        ...
        cv2.rectangle(passedframe, (x, y), (x + w, y + h), color_dict[label], 2)
        cv2.rectangle(passedframe, (x, y - 40), (x + w, y), color_dict[label], -1)
        cv2.putText(passedframe, mask_labels_dict[label] + " " + str(percent) + "%", (x, y - 10),
...
elif radio == 'Live(Available when you run it on your own PC*)':
    with col1:
        st.subheader('Live Feed')
        FRAMES = st.image([])
        cam = cv2.VideoCapture(0)
        while radio == 'Live(Available when you run it on your own PC*)':
            ret, frame = cam.read()
